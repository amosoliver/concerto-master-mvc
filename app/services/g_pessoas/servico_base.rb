module GPessoas
  class ServicoBase
    ...
    def valid_for_step?(step, clear_errors: true)
      g_pessoa.errors.clear if clear_errors
      ...
      when "dados"
        ...
        g_pessoa.valid?
      when "grupos"
        validate_presence_ids!(:m_grupo_ids, "Selecione ao menos um grupo.")
      when "instrumentos"
        validate_presence_ids!(:g_instrumento_naipe_ids, "Selecione ao menos um instrumento/naipe.")
      when "funcoes"
        validate_funcoes_step
      when "usuario"
        validate_presence_ids!(:u_perfil_ids, "Selecione ao menos um perfil.")
      else
        true
      end
    end
    ...
    def validate_presence_ids!(key, message)
      return true if normalize_ids(params[key]).any?

      g_pessoa.errors.add(:base, message)
      false
    end

    def validate_funcoes_step
      funcao_ids = normalize_ids(params[:u_funcao_ids])

      if funcao_ids.empty?
        g_pessoa.errors.add(:base, "Selecione ao menos uma função.")
        return false
      end

      principal_id = params[:principal_u_funcao_id].presence&.to_i
      return true if principal_id.present? && funcao_ids.include?(principal_id)

      g_pessoa.errors.add(:base, "Selecione a função principal entre as funções marcadas.")
      false
    end
  end
end
