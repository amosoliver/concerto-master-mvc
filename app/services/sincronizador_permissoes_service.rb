...
class SincronizadorPermissoesService
  ...
  def call
    ...
    UPermissao.transaction do
      total_alterado = sync_permissions(route_permissions)
      sync_admin_profile_permissions!
    end

    total_alterado
  rescue ActiveRecord::ConnectionNotEstablished, ActiveRecord::NoDatabaseError, ActiveRecord::StatementInvalid, PG::Error => error
    Rails.logger.warn("SincronizadorPermissoesService: sincronização ignorada (#{error.class}: #{error.message})")
    0
  end
  ...
  def perfil_table_ready?
    ActiveRecord::Base.connection.data_source_exists?("u_perfis")
  end
  ...
  def sync_permissions(route_permissions)
    ...
    active_pairs = route_permissions.map { |attributes| [attributes[:controlador], attributes[:acao]] }.to_set
    ...
    UPermissao.find_each do |permission|
      next if active_pairs.include?([permission.controlador, permission.acao])
  ...
  def sync_admin_profile_permissions!
    return unless perfil_table_ready?

    admin_profile = UPerfil.with_discarded.find_or_initialize_by(descricao: ADMIN_PROFILE_NAME)
    admin_profile.deleted_at = nil
    admin_profile.save! if admin_profile.changed?
    admin_profile.sync_u_permissoes!(UPermissao.pluck(:id))
  end
end
