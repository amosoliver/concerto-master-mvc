class GSexosController < ApplicationController
  ...
  def create
    ...
    if @g_sexo.save
      redirect_to @g_sexo, notice: "G sexo criado com sucesso."
    else
  ...
  def update
    if @g_sexo.update(g_sexo_params)
      redirect_to @g_sexo, notice: "G sexo atualizado com sucesso."
    else
  ...
  def destroy
    @g_sexo.discard
    redirect_to g_sexos_path, notice: "G sexo removido com sucesso."
  end
