class MPessoasFuncoesController < ApplicationController
  ...
  def create
    ...
    if @m_pessoa_funcao.save
      redirect_to m_pessoas_funcoes_path, notice: "#{MPessoaFuncao.model_name.human} criado com sucesso."
    else
  ...
  def update
    if @m_pessoa_funcao.update(m_pessoa_funcao_params)
      redirect_to m_pessoas_funcoes_path, notice: "#{MPessoaFuncao.model_name.human} atualizado com sucesso."
    else
