class GPaisesController < ApplicationController
  ...
  def create
    ...
    if @g_pais.save
      redirect_to @g_pais, notice: "G pais criado com sucesso."
    else
  ...
  def update
    if @g_pais.update(g_pais_params)
      redirect_to @g_pais, notice: "G pais atualizado com sucesso."
    else
  ...
  def destroy
    @g_pais.discard
    redirect_to g_paises_path, notice: "G pais removido com sucesso."
  end
