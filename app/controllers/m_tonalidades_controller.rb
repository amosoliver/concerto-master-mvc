class MTonalidadesController < ApplicationController
  ...
  def create
    ...
    if @m_tonalidade.save
      redirect_to m_tonalidades_path, notice: "#{MTonalidade.model_name.human} criado com sucesso."
    else
  ...
  def update
    if @m_tonalidade.update(m_tonalidade_params)
      redirect_to m_tonalidades_path, notice: "#{MTonalidade.model_name.human} atualizado com sucesso."
    else
