class GInstrumentosController < ApplicationController
  ...
  def create
    ...
    if @g_instrumento.save
      redirect_to g_instrumentos_path, notice: "#{GInstrumento.model_name.human} criado com sucesso."
    else
  ...
  def update
    if @g_instrumento.update(g_instrumento_params)
      redirect_to g_instrumentos_path, notice: "#{GInstrumento.model_name.human} atualizado com sucesso."
    else
