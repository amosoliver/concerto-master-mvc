class GPrediosController < ApplicationController
  ...
  def create
    ...
    if @g_predio.save
      redirect_to g_predios_path, notice: "#{GPredio.model_name.human} criado com sucesso."
    else
  ...
  def update
    if @g_predio.update(g_predio_params)
      redirect_to g_predios_path, notice: "#{GPredio.model_name.human} atualizado com sucesso."
    else
