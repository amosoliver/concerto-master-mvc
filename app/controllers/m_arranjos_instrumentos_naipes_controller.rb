class MArranjosInstrumentosNaipesController < ApplicationController
  ...
  def create
    ...
    if @m_arranjo_instrumento_naipe.save
      redirect_to m_arranjos_instrumentos_naipes_path, notice: "#{MArranjoInstrumentoNaipe.model_name.human} criado com sucesso."
    else
  ...
  def update
    if @m_arranjo_instrumento_naipe.update(m_arranjo_instrumento_naipe_params)
      redirect_to m_arranjos_instrumentos_naipes_path, notice: "#{MArranjoInstrumentoNaipe.model_name.human} atualizado com sucesso."
    else
