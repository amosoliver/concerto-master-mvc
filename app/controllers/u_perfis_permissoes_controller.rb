class UPerfisPermissoesController < ApplicationController
  ...
  def create
    ...
    if @u_perfil_permissao.save
      redirect_to u_perfis_permissoes_path, notice: "#{UPerfilPermissao.model_name.human} criado com sucesso."
    else
  ...
  def update
    if @u_perfil_permissao.update(u_perfil_permissao_params)
      redirect_to u_perfis_permissoes_path, notice: "#{UPerfilPermissao.model_name.human} atualizado com sucesso."
    else
