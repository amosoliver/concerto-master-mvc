class MEventosController < ApplicationController
  ...
  def create
    ...
    if @m_evento.save
      redirect_to m_eventos_path, notice: "#{MEvento.model_name.human} criado com sucesso."
    else
  ...
  def update
    if @m_evento.update(m_evento_params)
      redirect_to m_eventos_path, notice: "#{MEvento.model_name.human} atualizado com sucesso."
    else
