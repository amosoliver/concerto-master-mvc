class MCompositoresController < ApplicationController
  ...
  def create
    ...
    if @m_compositor.save
      redirect_to m_compositores_path, notice: "#{MCompositor.model_name.human} criado com sucesso."
    else
  ...
  def update
    if @m_compositor.update(m_compositor_params)
      redirect_to m_compositores_path, notice: "#{MCompositor.model_name.human} atualizado com sucesso."
    else
