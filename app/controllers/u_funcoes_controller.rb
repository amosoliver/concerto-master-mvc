class UFuncoesController < ApplicationController
  ...
  def create
    ...
    if @u_funcao.save
      redirect_to u_funcoes_path, notice: "#{UFuncao.model_name.human} criado com sucesso."
    else
  ...
  def update
    if @u_funcao.update(u_funcao_params)
      redirect_to u_funcoes_path, notice: "#{UFuncao.model_name.human} atualizado com sucesso."
    else
